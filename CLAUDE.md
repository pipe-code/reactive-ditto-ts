...
- **`window.gtag` must actually be global, not just a local function.** `header.php`'s GA4 snippet declares `function gtag(){dataLayer.push(arguments);}` *and* sets `window.gtag = gtag;` right after — that second line is not decorative. Miss it and `gtag('js', …)`/`gtag('config', …)` inside that same closure still work (they call the local function directly), but every `gtag('event', …)` call from `tracking.tsx` silently no-ops (`typeof window.gtag === 'function'` is false), which looks — from inside the app — exactly like the SPA-navigation bug above even after "fixing" it. Confirmed on a live site: `dataLayer.push()` and `fbq()` calls fired correctly, `gtag()` calls didn't, until this line was added. Verify with `typeof window.gtag === 'function'` in the console, not just by reading the dataLayer.
